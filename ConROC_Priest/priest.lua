...
local Racial, Spec, Caster, Ability, Rank, Disc_Talent, Holy_Talent, Shad_Talent, Buff, Debuff = ids.Racial, ids.Spec, ids.Caster, ids.Ability, ids.Rank, ids.Discipline_Talent, ids.Holy_Talent, ids.Shadow_Talent, ids.Buff, ids.Debuff;
...
--Abilities
	...
	local _Shadowfiend, _Shadowfiend_RDY = ConROC:AbilityReady(Ability.Shadowfiend, timeShift);
	local _ShadowWordDeath, _ShadowWordDeath_RDY = ConROC:AbilityReady(Ability.ShadowWordDeath, timeShift);
...
--Rotations
	repeat
		while(true) do
			-- Enter Shadowform if available and not currently active
			...
			if _Shadowform_FORM then
				...
				-- Shadow Word: Death (damage + execute range, risky below 50% health)
				...
				-- Mind Blast (core damage, use on cooldown)
				if _MindBlast_RDY and currentSpell ~= _MindBlast then
					tinsert(ConROC.SuggestedSpells, _MindBlast);
					_MindBlast_RDY = false;
					_Queue = _Queue + 1;
			...
			-- Non-Shadow / no Shadowform: Holy or Disc DPS rotation
...
--Abilities
	...
	local _PowerWordFortitude, _PowerWordFortitude_RDY = ConROC:AbilityReady(Ability.PowerWordFortitude, timeShift);
		local _PowerWordFortitude_BUFF = ConROC:Aura(_PowerWordFortitude, timeShift);
	local _PrayerofFortitude, _PrayerofFortitude_RDY = ConROC:AbilityReady(Ability.PrayerofFortitude, timeShift);
		local _PrayerofFortitude_BUFF = ConROC:Aura(_PrayerofFortitude, timeShift);
