...
--Abilities
	...
	local _DragonBreath, _DragonBreath_RDY = ConROC:AbilityReady(Ability.DragonBreath, timeShift);
...
--Indicators
	...
	ConROC:AbilityBurst(_IcyVeins, _IcyVeins_RDY and _in_combat);
	...
	repeat
		while(true) do
			...
			if ConROC.Seasons.IsSoD then -- SoD rotation (dead code on TBC servers)
				...
				-- Icy Veins: use on cooldown in combat (Frost)
				if ConROC:CheckBox(ConROC_SM_CD_IcyVeins) and _IcyVeins_RDY and _in_combat then
				...
				-- Refresh Winter's Chill before it falls off
				...
				-- Refresh Fire Vulnerability before it falls off
				...
				-- AoE rotation
				if ConROC_AoEButton:IsVisible() then
					-- Dragon's Breath (Fire AoE, melee range)
					if ConROC:CheckBox(ConROC_SM_AoE_DragonBreath) and _DragonBreath_RDY and _target_in_melee then
						tinsert(ConROC.SuggestedSpells, _DragonBreath);
						_DragonBreath_RDY = false;
						_Queue = _Queue + 1;
						break;
					end
				...
				-- Cone of Cold: use on a Frost Nova'd target in melee range
				...
				-- Combustion: use when Fire Vulnerability is at 5 stacks
				...
				-- Build Fire Vulnerability stacks (Improved Scorch)
...
--Abilities
	...
	local _IceBlock, _IceBlock_RDY = ConROC:AbilityReady(Ability.IceBlock, timeShift);
	local _FrostNova, _FrostNova_RDY = ConROC:AbilityReady(Ability.FrostNova, timeShift);
...
--Rotations
	...
	if _IceBlock_RDY and _Player_Percent_Health <= 10 then
		tinsert(ConROC.SuggestedDefSpells, _IceBlock);
	end
