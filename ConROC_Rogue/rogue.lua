...
--Abilities
    ...
    local _Evasion, _Evasion_RDY = ConROC:AbilityReady(Ability.Evasion, timeShift);  -- For Preparation logic
...
--Conditions
    ...
    -- Detect if player has Hemorrhage talent (Subtlety)
    local _Has_Hemorrhage_Talent = ConROC:TalentChosen(Spec.Subtlety, Sub_Talent.Hemorrhage);
 ...
 --Indicators
    ...
    -- Preparation: Suggest when important cooldowns are down (Vanish, Evasion, Shadowstep, Cold Blood, Sprint)
    -- Prioritize Vanish and Evasion as they're the most critical defensive cooldowns
    local _Preparation_Value = (_Vanish_RDY == false) or 
                                (_Evasion_RDY == false and _Player_Percent_Health <= 80) or
                                (_Shadowstep_RDY == false and _target_in_25yrds and not _target_in_melee) or
                                (_ColdBlood_RDY == false and _Combo >= 4) or
                                (_Sprint_RDY == false and _is_moving)
    ConROC:AbilityBurst(_Preparation, _Preparation_RDY and _in_combat and _Preparation_Value)
            ...
            if ConROC.Seasons.IsSoD then
                if ConROC:CheckBox(ConROC_SM_Role_Tank) or ConROC:RuneEquipped(ids.Engrave.JustaFleshWound, "chest") then
                    ...
                    if _is_stealthed then
                        ...
                        -- Premeditation: Only suggest on NPCs (not players), preferably elites/bosses
                        if _Premeditation_RDY and not _is_PC then
                ...
                if _is_stealthed then
                    ...
                    -- Backstab from stealth: Only if not using Hemorrhage (Ambush is better for stealth openers)
                    -- Will NOT use if: (1) You have Hemorrhage talent, OR (2) Hemorrhage checkbox is enabled
                    if not _Has_Hemorrhage_Talent and not ConROC:CheckBox(ConROC_SM_Debuff_Hemorrhage) and _Backstab_RDY and _Energy >= _Backstab_COST and hasDagger and not ConROC:TarYou() then
                        tinsert(ConROC.SuggestedSpells, _Backstab);
                        _is_stealthed = false;
                        _Energy = _Energy - _Backstab_COST;
                        _Combo = _Combo + 1;
                ...
                -- Hemorrhage: Primary combo builder for Subtlety spec (35 energy, applies debuff)
                -- Will use if: (1) You have the talent, OR (2) The checkbox is enabled
                if (_Has_Hemorrhage_Talent or ConROC:CheckBox(ConROC_SM_Debuff_Hemorrhage)) and _Hemorrhage_RDY and _Energy >= 35 and _Combo <= _Combo_Max - 1 then
                    tinsert(ConROC.SuggestedSpells, _Hemorrhage);
                    if not _Hemorrhage_DEBUFF then
                        _Hemorrhage_DEBUFF = true;
                    end
                    _Combo = _Combo + 1;
                    _Energy = _Energy - 35;
                ...
                -- Backstab: Only use if not using Hemorrhage (requires being behind target, 60 energy)
                -- For Subtlety/Hemorrhage spec, Hemorrhage is much better (35 energy, no positioning needed)
                -- Will NOT use if: (1) You have Hemorrhage talent, OR (2) Hemorrhage checkbox is enabled
                if not _Has_Hemorrhage_Talent and not ConROC:CheckBox(ConROC_SM_Debuff_Hemorrhage) and _Backstab_RDY and _Energy >= _Backstab_COST and hasDagger and not ConROC:TarYou() then
                    tinsert(ConROC.SuggestedSpells, _Backstab);
                    _Energy = _Energy - _Backstab_COST;
