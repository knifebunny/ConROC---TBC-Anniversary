...
--Indicators
	...
	ConROC:AbilityBurst(_DeathCoil, _DeathCoil_RDY and _is_PvP and _enemies_in_melee >= 1 and _Player_Percent_Health < 60);
	ConROC:AbilityBurst(_Fear, _Fear_RDY and _is_PvP and _enemies_in_melee >= 1 and not _Fear_DEBUFF);
	ConROC:AbilityBurst(_HowlofTerror, _HowlofTerror_RDY and _is_PvP and _enemies_in_melee >= 2);
	...
	repeat
		while(true) do
			...
			if ConROC.Seasons.IsSoD then
				...
				if _is_PvP and _is_PC and _is_Enemy then
				...
				--Curses--
				-- Amplify Curse (PVP priority)
				if _is_PvP and _is_PC and _is_Enemy and _AmplifyCurse_RDY and (
					(ConROC:CheckBox(ConROC_SM_Curse_Tongues) and not _CurseofTongues_DEBUFF and tarHasMana > 0) or
					(ConROC:CheckBox(ConROC_SM_Curse_Exhaustion) and not _CurseofExhaustion_DEBUFF) or
					(ConROC:CheckBox(ConROC_SM_Curse_Agony) and not _CurseofAgony_DEBUFF)
				) then
				...
				-- Curse of Tongues (PVP vs casters)
				if _is_PvP and _is_PC and _is_Enemy and ConROC:CheckBox(ConROC_SM_Curse_Tongues) and _CurseofTongues_RDY and not _CurseofTongues_DEBUFF and tarHasMana > 0 then
					tinsert(ConROC.SuggestedSpells, _CurseofTongues);
					_CurseofTongues_DEBUFF = true;
					_Queue = _Queue + 1;
					break;
				...
				-- Curse of Exhaustion (PVP vs melee)
				if _is_PvP and _is_PC and _is_Enemy and ConROC:CheckBox(ConROC_SM_Curse_Exhaustion) and _CurseofExhaustion_RDY and not _CurseofExhaustion_DEBUFF and tarHasMana == 0 then
					tinsert(ConROC.SuggestedSpells, _CurseofExhaustion);
					_CurseofExhaustion_DEBUFF = true;
					_Queue = _Queue + 1;
					break;
				...
				-- Regular curse rotation (PVE or PVP fallback)
				if ConROC:CheckBox(ConROC_SM_Curse_Weakness) and _CurseofWeakness_RDY and not _CurseofWeakness_DEBUFF and ((ConROC:Raidmob() and _Target_Percent_Health >= 5) or (not ConROC:Raidmob() and _Target_Percent_Health >= 20)) then
					tinsert(ConROC.SuggestedSpells, _CurseofWeakness);
					_CurseofWeakness_DEBUFF = true;
					_Queue = _Queue + 1;
				...
				if _is_PvP and _is_PC and _is_Enemy then
					...
					if ConROC:CheckBox(ConROC_SM_Debuff_UnstableAffliction) and _UnstableAffliction_RDY and not _UnstableAffliction_DEBUFF and not _Immolate_DEBUFF and ((ConROC:Raidmob() and _Target_Percent_Health >= 5) or (not ConROC:Raidmob() and _Target_Percent_Health >= 20)) then
						tinsert(ConROC.SuggestedSpells, _UnstableAffliction);
						_UnstableAffliction_DEBUFF = true;
						_Queue = _Queue + 1;
						break;
