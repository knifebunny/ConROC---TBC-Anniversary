...
function ConROC:forceNameplates()
    ...
    if defaultEnemyNameplates ~= "1" then
        ...
        SetCVar("nameplateNotSelectedAlpha", ConROC.db.profile.nameplateNotSelectedAlpha or defaultNameplateNotSelectedAlpha)
        SetCVar("nameplateSelectedAlpha",    ConROC.db.profile.nameplateSelectedAlpha    or defaultNameplateSelectedAlpha)
        SetCVar("nameplateShowEnemies", "1")
    end
end

function ConROC:restoreNameplates()
    if defaultNameplateNotSelectedAlpha then
        SetCVar("nameplateNotSelectedAlpha", defaultNameplateNotSelectedAlpha)
    end
    if defaultNameplateSelectedAlpha then
        SetCVar("nameplateSelectedAlpha", defaultNameplateSelectedAlpha)
    end
    if defaultEnemyNameplates then
...
-- Simple, class-safe GCD check: use a known instant spell for your class.
-- For Warrior, 78 = Heroic Strike (works as a GCD proxy).
ConROC.GCDProbe = 78

function ConROC:IsOnGCD()
    local probe = ConROC.GCDProbe
    local start, dur = GetSpellCooldown(probe)
    -- GCD typically <= 1.5s; ignore longer CDs
    return (start and dur and dur > 0 and dur <= 1.6) or false
end

-- Current stance: 1 = Battle, 2 = Defensive, 3 = Berserker
function ConROC:GetStance()
    local form = GetShapeshiftForm()
    return form or 0
end

-- Does this spell require a particular stance? (Warrior examples)
-- Add/adjust as you like.
ConROC.WarriorStanceReq = ConROC.WarriorStanceReq or {
    [7384] = 1,    -- Overpower → Battle
    [1464] = 1,    -- Slam → Battle (in Classic)
    [355]  = 2,    -- Taunt → Defensive
    [2565] = 2,    -- Shield Block → Defensive
    [3411] = 2,    -- Intervene (TBC+) ignore if not in your branch
    [20252] = 3,   -- Intercept → Berserker
    [1680] = 3,    -- Whirlwind → Berserker
    -- add more as needed
}

function ConROC:SpellRequiresDifferentStance(spellID)
    local need = ConROC.WarriorStanceReq[spellID]
    if not need or need == 0 then return false, nil end
    local have = ConROC:GetStance()
    return have ~= need, need
end

-- Map stance id to icon
ConROC.WarriorStanceIcon = ConROC.WarriorStanceIcon or {
    [1] = "Interface\\Icons\\Ability_Warrior_OffensiveStance",
    [2] = "Interface\\Icons\\Ability_Warrior_DefensiveStance",
    [3] = "Interface\\Icons\\Ability_Warrior_BerserkerStance",
}

-- Range test wrapper you already have, normalized to boolean
function ConROC:IsInRangeBool(spellID, unit)
    local r = self:IsSpellInRange(spellID, unit or "target")
    return r == true
end
...
function ConROC:EndCast(target)
    ...
    -- Use the same probe as IsOnGCD (Classic safe)
    if target == 'player' then
        local probe = ConROC.GCDProbe or 78 -- Heroic Strike as default
        local gstart, gduration = GetSpellCooldown(probe)
        gcd = (gduration or 0) - (t - (gstart or 0))
...
function ConROC:IsOverride(spellID)
    local _OverriddenBy = C_Spell and C_Spell.GetOverrideSpell and C_Spell.GetOverrideSpell(spellID)
...
function ConROC:IsSpellInRange(spellid, target_unit)
	...
	-- Only bother if it's a hostile target and we actually know the spell
	if not ConROC:TarHostile() then return false end

	-- “IsPlayerSpell” exists in Classic; “IsSpellKnownOrOverridesKnown” may not, so guard it.
	...
	-- Classic-safe path: by spell *name*
	local name = GetSpellInfo(spellid)
	if name then
		-- Classic returns 1 (in range), 0 (out), or nil (can’t check)
		local res = IsSpellInRange(name, unit)
		if res == 1 then
			return true
		elseif res == 0 then
			return false
	...
	-- If the client happens to have the Retail API, allow it as a secondary check
	if C_Spell and C_Spell.IsSpellInRange then
		local inRange = C_Spell.IsSpellInRange(spellid, unit) -- true/false/nil
		return inRange == true
	end
...
function ConROC:AbilityReady(spellid, timeShift, spelltype)
    ...
    -- Known check (Classic-safe)
    local known = IsPlayerSpell and IsPlayerSpell(spellid) or false
    if not known and IsSpellKnownOrOverridesKnown then
        known = IsSpellKnownOrOverridesKnown(spellid)
    end
    if spelltype == 'pet' then
        known = IsSpellKnown and IsSpellKnown(spellid, true) or known
    end

    -- Usable check (guard Retail API)
    local usable, notEnough
    if C_Spell and C_Spell.IsSpellUsable then
        usable, notEnough = C_Spell.IsSpellUsable(spellid)
    else
        usable, notEnough = IsUsableSpell(spellid)  -- Classic fallback
    end

    -- Cast time (guard Retail API)
    local castTimeMilli
    if C_Spell and C_Spell.GetSpellInfo then
        local info = C_Spell.GetSpellInfo(spellid)
        castTimeMilli = info and info.castTime or 0
    else
        local _, _, _, castMS = GetSpellInfo(spellid) -- 4th return is cast time (ms)
        castTimeMilli = castMS or 0
    end
    local castTime = castTimeMilli / 1000

    local rdy
    if spelltype == 'known' then
        rdy = known and _CD <= 0
    else
        rdy = known and usable and _CD <= 0 and not notEnough
    end
