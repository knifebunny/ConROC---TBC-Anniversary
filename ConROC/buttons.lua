...
SLASH_CONROCTOGGLE1 = '/ConROCToggle'
SlashCmdList["CONROCTOGGLE"] = function()
    if ConROC_SingleButton:IsShown() then
...
function ConROC:FindKeybinding(id,caller)
	local keybind;
	-- Use name-based matching so the correct keybind shows even when a different
	-- rank of the spell is on the action bar than what the rotation suggested.
	local matchedID = self:FindMatchingSpellID(id, self.Keybinds);
	if matchedID ~= nil then
		for k, button in pairs(self.Keybinds[matchedID]) do
			for i = 1, 12 do
...
-- Returns the spell ID that is actually on the action bar whose spell name
-- matches the requested spellID. This allows any rank of a spell to satisfy
-- the rotation suggestion, so players can cast downranked spells for mana
-- efficiency without the rotation getting stuck.
function ConROC:FindMatchingSpellID(spellID, spellTable)
	-- First try an exact match
	if spellTable[spellID] ~= nil then
		return spellID;
	end

	-- Fall back to name-based match across all registered spells
	local requestedName = GetSpellInfo(spellID);
	if requestedName then
		for registeredID, _ in pairs(spellTable) do
			local registeredName = GetSpellInfo(registeredID);
			if registeredName == requestedName then
				return registeredID;
			end
		end
	end
...
function ConROC:GlowSpell(spellID)
	...
	-- Use name-based matching so any rank of the suggested spell satisfies the
	-- rotation. This allows downranking for mana efficiency without getting stuck.
	local matchedID = self:FindMatchingSpellID(spellID, self.Spells);
	if matchedID ~= nil then
		for k, button in pairs(self.Spells[matchedID]) do
			self:DamageGlow(button, 'next');
		end
		self.SpellsGlowing[matchedID] = 1;
	else
