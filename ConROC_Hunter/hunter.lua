...
local Racial, Spec, Ability, Rank, BM_Talent, MM_Talent, Surv_Talent, Pet, Buff, Debuff = ids.Racial, ids.Spec, ids.Ability, ids.Rank, ids.BeastMastery_Talent, ids.Marksmanship_Talent, ids.Survival_Talent, ids.Pet, ids.Buff, ids.Debuff;
...
--Abilities
	...
	local _ScorpidSting, _ScorpidSting_RDY = ConROC:AbilityReady(Ability.ScorpidSting, timeShift);
		local _ScorpidSting_DEBUFF = ConROC:TargetAura(_ScorpidSting, timeShift);
	local _SerpentSting, _SerpentSting_RDY = ConROC:AbilityReady(Ability.SerpentSting, timeShift);
		local _SerpentSting_DEBUFF = ConROC:TargetAura(_SerpentSting, timeShift);
	...
	local _ViperSting, _ViperSting_RDY = ConROC:AbilityReady(Ability.ViperSting, timeShift);
		local _ViperSting_DEBUFF = ConROC:TargetAura(_ViperSting, timeShift);
...
--Conditions
	...
	local tarHasMana = UnitPower('target', 0); -- 0 = Mana power type (Classic/TBC API)
...
-- Called when the player successfully casts a spell; used to track Auto Shot state.
function ConROC:JustCasted(spellID)
	if spellID == 75 then -- 75 = Auto Shot
		_AutoShot_ACTIVE = true;
	end
end
