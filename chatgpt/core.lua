...
-- === UI polish widgets & helpers (stance overlay + tint/dim) ===
function ConROC:EnsureUIPolishWidgets()
    if ConROC.__UIPolishReady then return end
    -- We anchor to ConROCWindow, which your code creates in DisplayWindowFrame()
    if not ConROCWindow or not ConROCWindow.texture then return end

    -- Tiny stance badge in bottom-left of the main suggestion window
    if not ConROC.StanceOverlay then
        local tex = ConROCWindow:CreateTexture(nil, "OVERLAY")
        tex:SetSize(20, 20)
        tex:SetPoint("BOTTOMLEFT", ConROCWindow, "BOTTOMLEFT", -2, -2)
        tex:Hide()
        ConROC.StanceOverlay = tex
    end

    function ConROC:SetSuggestVisual(r, g, b, a)
        -- tint the main icon
        if ConROCWindow and ConROCWindow.texture then
            ConROCWindow.texture:SetVertexColor(r or 1, g or 1, b or 1)
        end
        -- set frame alpha (separate so we can dim for GCD)
        if ConROCWindow then
            ConROCWindow:SetAlpha(a or 1)
        end
    end

    ConROC.__UIPolishReady = true
end

-- Post-suggest UI hook; call this after picking the best spell
function ConROC:PostSuggestUI(spellID, unit)
    unit = unit or "target"
    ConROC:EnsureUIPolishWidgets()
    if not ConROC.__UIPolishReady then return end

    -- defaults
    local alpha = 1

    -- 1) Range tint (needs helper.lua IsInRangeBool)
    local inRange = ConROC.IsInRangeBool and ConROC:IsInRangeBool(spellID, unit)
    if inRange == false then
        ConROC:SetSuggestVisual(1, 0.35, 0.35, alpha)  -- reddish when OOR
    else
        ConROC:SetSuggestVisual(1, 1, 1, alpha)        -- normal
    end

    -- 2) Dim while on GCD (needs helper.lua IsOnGCD)
    local onGCD = ConROC.IsOnGCD and ConROC:IsOnGCD()
    if onGCD then
        ConROCWindow:SetAlpha(0.60)
    end

    -- 3) Stance badge (needs helper.lua SpellRequiresDifferentStance + WarriorStanceIcon)
    if ConROC.SpellRequiresDifferentStance and ConROC.WarriorStanceIcon then
        local needsDifferent, neededStance = ConROC:SpellRequiresDifferentStance(spellID)
        if needsDifferent and ConROC.WarriorStanceIcon[neededStance] then
            ConROC.StanceOverlay:SetTexture(ConROC.WarriorStanceIcon[neededStance])
            ConROC.StanceOverlay:Show()
        elseif ConROC.StanceOverlay then
            ConROC.StanceOverlay:Hide()
        end
    end
end
-- === end UI polish ===
...
function ConROC:InvokeNextSpell()
    ...
    -- Resolve spell textures/names (guard C_Spell for Classic branches)
    local spellName, spellTexture;

    if self.Spell then
        local spellInfo1 = (C_Spell and C_Spell.GetSpellInfo and C_Spell.GetSpellInfo(self.Spell)) or nil
        if spellInfo1 then
            spellName = spellInfo1.name
            spellTexture = spellInfo1.originalIconID
        else
            -- fallback if needed
            spellName = GetSpellInfo(self.Spell)
            spellTexture = select(3, GetSpellInfo(self.Spell))
        end
    end

    if self.Spell == 26008 then -- "Wait" sentinel
        spellName = "Wait";
    end

    local spellTexture2
    if self.SuggestedSpells[2] then
        local si2 = (C_Spell and C_Spell.GetSpellInfo and C_Spell.GetSpellInfo(self.SuggestedSpells[2])) or nil
        spellTexture2 = si2 and si2.originalIconID or select(3, GetSpellInfo(self.SuggestedSpells[2]))
    end

    local spellTexture3
    if self.SuggestedSpells[3] then
        local si3 = (C_Spell and C_Spell.GetSpellInfo and C_Spell.GetSpellInfo(self.SuggestedSpells[3])) or nil
        spellTexture3 = si3 and si3.originalIconID or select(3, GetSpellInfo(self.SuggestedSpells[3]))
    end

    -- Only do the heavy UI swap when the suggested spell actually changes
    ...
    -- ✅ Always run UI polish every tick while a spell is suggested
    if self.Spell then
        ConROC:PostSuggestUI(self.Spell, "target")
    end

    -- When no suggestion, clear & reset polish (this stays INSIDE the function)
    if self.Spell == nil and oldSkill ~= nil then
        ...
        -- Reset the tint/dim/stance badge
        ConROC:EnsureUIPolishWidgets()
        if ConROC.__UIPolishReady then
            ConROC:SetSuggestVisual(1, 1, 1, 1)
            if ConROC.StanceOverlay then ConROC.StanceOverlay:Hide() end
        end
    end
end
...
if self.Spell == nil and oldSkill ~= nil then
    ...
    -- ✅ reset polish ONLY when no suggestion
    ConROC:EnsureUIPolishWidgets()
    if ConROC.__UIPolishReady then
        ConROC:SetSuggestVisual(1, 1, 1, 1)
        if ConROC.StanceOverlay then ConROC.StanceOverlay:Hide() end
