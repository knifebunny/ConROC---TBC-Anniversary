...
local defaults = {
	...
	["ConROC_Caster_DoT_InsectSwarm"] = true,
	["ConROC_Caster_DoT_Rake"] = false,
	["ConROC_Caster_DoT_Rip"] = false,
	["ConROC_Caster_DoT_Lacerate"] = false,
	["ConROC_Caster_Ability_Shred"] = false,
	["ConROC_Caster_Ability_FerociousBite"] = false,
	...
	["ConROC_Caster_CD_ForceofNature"] = false,
	["ConROC_Caster_CD_Barkskin"] = false,
	["ConROC_Caster_Option_AoE"] = false,

	["ConROC_SM_Role_Melee"] = false,
	["ConROC_Melee_DoT_Moonfire"] = false,
	["ConROC_Melee_DoT_InsectSwarm"] = false,
	["ConROC_Melee_DoT_Rake"] = true,
	["ConROC_Melee_DoT_Rip"] = true,
	["ConROC_Melee_DoT_Lacerate"] = false,
	["ConROC_Melee_Ability_Shred"] = true,
	["ConROC_Melee_Ability_FerociousBite"] = true,
	...
	["ConROC_Melee_CD_ForceofNature"] = false,
	["ConROC_Melee_CD_Barkskin"] = false,
	["ConROC_Melee_Option_AoE"] = false,

	["ConROC_SM_Role_Tank"] = false,
	["ConROC_Tank_DoT_Moonfire"] = false,
	["ConROC_Tank_DoT_InsectSwarm"] = false,
	["ConROC_Tank_DoT_Rake"] = false,
	["ConROC_Tank_DoT_Rip"] = false,
	["ConROC_Tank_DoT_Lacerate"] = true,
	["ConROC_Tank_Ability_Shred"] = false,
	["ConROC_Tank_Ability_FerociousBite"] = false,
	...
	["ConROC_Tank_CD_ForceofNature"] = false,
	["ConROC_Tank_CD_Barkskin"] = true,
	["ConROC_Tank_Option_AoE"] = false,

	["ConROC_SM_Role_Healer"] = false,
	["ConROC_Healer_DoT_Moonfire"] = false,
	["ConROC_Healer_DoT_InsectSwarm"] = false,
	["ConROC_Healer_DoT_Rake"] = false,
	["ConROC_Healer_DoT_Rip"] = false,
	["ConROC_Healer_DoT_Lacerate"] = false,
	["ConROC_Healer_Ability_Shred"] = false,
	["ConROC_Healer_Ability_FerociousBite"] = false,
	...
	["ConROC_Healer_CD_ForceofNature"] = false,
...
function ConROC:setRole(radioBtn, roleData, radioButtons)
	for _, btn in ipairs(radioButtons) do
    	...
    	ConROCDruidSpells[btn.role] = false
    end
    radioBtn:SetChecked(true)
    ConROCDruidSpells[roleData.role] = true
end
...
function ConROC:setRoleChecked(_spellData, _oItem)
	...
	if ConROC:CheckBox(activeRole) then
		...
		if _spellData.type == "textfield" then
			_oItem:SetNumber(ConROCDruidSpells[spellCheck]);
		else
			_oItem:SetChecked(ConROCDruidSpells[spellCheck]);
		end
	end
end
...
function ConROC:setRoleSpellClicked(_spellData, _oItem)
	...
	if ConROC:CheckBox(activeRole) then
		local spellCheck = checkboxName .. _spellData.spellCheckbox
		if _spellData.type == "textfield" then
			ConROCDruidSpells[spellCheck] = _G["ConROC_SM_".._spellData.spellCheckbox]:GetNumber();
		else
			ConROCDruidSpells[spellCheck] = _oItem:GetChecked();
		end
	end
end
	...
	ConROC_RotationSettingsTable = {
		{
			...
			spells = {
				...
				{spellID = ids.Ability.InsectSwarm, spellCheckbox = "DoT_InsectSwarm", reqLevel = 20, type="spell"},
				...
				{spellID = ids.Ability.Lacerate, spellCheckbox = "DoT_Lacerate", reqLevel = 66, type="spell"},
			},
		...
		},
		{
			frameName = "Abilities",
			spells = {
				{spellID = ids.Ability.Shred, spellCheckbox = "Ability_Shred", reqLevel = 22, type="spell"},
				{spellID = ids.Ability.FerociousBite, spellCheckbox = "Ability_FerociousBite", reqLevel = 32, type="spell"},
			},
			groupType = "checkBoxes"
		},
		{
		...
		},
		{
	    	frameName = "Options",
	    	spells = {
		    	--{spellID = ids.Ability.Wrath, spellCheckbox = "Option_PrePull", reqLevel = 80, type="custom", icon=237511, customName="Use Prepull actions"},
...
function ConROC_roles(frame)
    ...
    for i, roleData in ipairs(ConROC_RoleSettingsTable) do
        ...
        radioBtn:SetChecked(ConROCDruidSpells[roleData.role])
		...
		function(self)
			...
			for _, radioButton in ipairs(_radioButtonsTable) do
				if radioButton ~= self then
					...
					ConROCDruidSpells[checkboxName .. radioButton.spellCheckbox] = radioButton:GetChecked()
				else
					self:SetChecked(true)
					ConROCDruidSpells[checkboxName .. radioButton.spellCheckbox] = self:GetChecked()
				end
			end
		end);
	oItemtext:SetText(spellName);
	...
	if ConROC:CheckBox(activeRole) then
	    for _, rotationSettings in ipairs(ConROC_RotationSettingsTable) do
	        for _, spellData in ipairs(rotationSettings.spells) do
	            ...
	            if ConROCDruidSpells[spellName] ~= nil then --and 
	            	if type(ConROCDruidSpells[spellName]) == "boolean" then
			            _G["ConROC_SM_" .. spellCheckbox]:SetChecked(ConROCDruidSpells[spellName])
	                elseif type(ConROCDruidSpells[spellName]) == "number" then
	                	_G["ConROC_SM_" .. spellCheckbox]:SetNumber(ConROCDruidSpells[spellName])
                	end
            	end
	        end
	    end
